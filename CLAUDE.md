...
**Wildcard products** sit alongside printed products: a `Product` with `is_wildcard = true` is a printless base (e.g. "Fold Hoodie Blank") whose stock acts as a shared pool. Other products in the same category set `wildcard_id` to point at it; the relation is strictly one level deep. On a Shopify push the wildcard's matching-`(color, size)` stock is folded into each derivative's effective quantity (`ShopifySyncService.computeEffectiveTotals`) — wildcards are never pushed independently. The conversion flow (`/stock/convert`, `WildcardService.convert`) writes a paired `−N` wildcard / `+N` derivative `StockMovement` in one transaction. A DB `CHECK (NOT (is_wildcard AND wildcard_id IS NOT NULL))` plus a same-category guard in `ProductsController` keep the relation coherent. UI surfaces the relationship with a 🃏 icon: on `/stock` printed rows show a `🃏 +N` pool chip via the server-computed `pools` prop (`WildcardService.computePoolMap`), and on `/sync` push/pull rows render the effective total as `{own} + 🃏 {pool}` (e.g. `2 + 🃏 10`) when the pool contributes — the tooltip key is `sync.wildcardPoolHint`.
...
- **Services** in `app/services/`: `StockService.adjust(...)` / `StockService.bulkAdjust(...)`, `VariantGenerator.generate(product, category, selection)`, `buildSkuCode(...)`, `WildcardService.convert(...)`. Controllers should never call models directly for stock writes — always go through `StockService`. Both `adjust` and `bulkAdjust` accept an optional `trx` so other services (`ShopifySyncService.applyPull`, `WildcardService.convert`) can atomically wrap stock changes with their own side-state updates. `bulkAdjust` skips rows whose quantity already matches `newQuantity`, so the grid-edit flow doesn't litter the audit history with no-op movements. The movement's `source` column is typed as the exported `MovementSource` union (`'manual' | 'shopify_pull' | 'shopify_push'`) — callers should import it instead of passing string literals. `WildcardService.convert` `firstOrCreate`s both stock rows and locks both with `forUpdate` _before_ computing `newQuantity` — the target row must not be read snapshot-only, or a concurrent adjuster could drift between the read and `StockService.adjust`'s own lock.
...
- **i18n** (`inertia/i18n/`) — `vue-i18n` (legacy mode off, `globalInjection` on) wired in `inertia/app.ts`. Strings live in `locales/en.json` and `locales/pt.json`; every page uses `useI18n()` and `$t(...)`. Locale is persisted to `localStorage` under `stockroom.locale` and falls back to the browser language. Use the existing keys and add new strings to **both** JSON files when adding UI copy.
- **Utilities** (`inertia/utils/`) — `shopify_image.ts` exports `shopifyThumb(url, width)`, which appends `?width=N` to a Shopify CDN URL so the CDN returns a downscaled image. Use it (directly or via `ImageZoom`) for any Shopify image render path — pulling the raw original shows up in perf traces as 100–400ms RasterTasks on the `/sync` tables.
- **Reusable components** (`inertia/components/`):
  ...
  - `MovementRow` — formatted history row used by both the global movements page and the product detail page. Leads with an `ImageZoom` thumbnail of the variant/product image when available.
  - `AttributeMultiSelect<T>` — pill-style multi-select used in product create/edit.
  - `VariantSelector` — cascading Location → Category → Product → Color/Size selector. Categories render via `CategoryPicker` (icon+label buttons using `category.icon` emoji), colors as swatches. After picking a category it auto-selects the first product, and after a product it auto-selects the first available color/size — so the user lands on a fully-resolved variant in one click. Accepts `initialVariant` + `initialLocationId` to restore state from a stock-row click. Defaults `locationId` to `locations[0]` when unset.
  - `VariantSummary` — display-only card for the resolved variant. Leads with an `ImageZoom` thumbnail when the variant/product has an image, falling back to size letter / color swatch as the icon. Shows attribute pills, location, and current on-hand. Used by the stock-adjust page.
  ...
  - `SyncVariantCell` — compact variant cell for the `/sync` tables: `ImageZoom` thumbnail + product name + color swatch chip + size box, matching the visual language used in `VariantSummary` / `VariantSelector`.
  - `ImageZoom` — thumbnail button that opens a teleported full-screen modal viewer (Esc / backdrop-click to close). Internally calls `shopifyThumb` for both the thumbnail (default `width=96`) and the full-screen render (default `width=1200`), so callers pass the raw Shopify CDN URL and the component picks sensible sizes. Used on every page that renders a product/variant image.
  - `WildcardModePicker` — radio-card picker on the product create/edit form for selecting wildcard mode (standalone / wildcard source / derived-from-wildcard). Dependent inputs render inlined inside the relevant card — the codebase's preferred pattern for mutually-exclusive form states.
  - `CategoryPicker` — icon+label button grid used inside `VariantSelector` and elsewhere for category selection.
  - `EmojiPicker` — emoji palette (backed by `emoji_sets.ts`) used when editing category icons.
  - `VariantTags` — pill display of a variant's attribute set (color/size badges).
  - `LanguageSwitcher` — locale toggle wired to `setLocale(...)` from `inertia/i18n/`.
...
- `ShopifySyncService` (`app/services/shopify_sync_service.ts`) — the core sync logic. Three apply methods (`applyLink`, `applyPush`, `applyPull`, plus `applyUnlink`) and matching `build*Diff` methods. `buildAllDiffs()` is the canonical entry point for `GET /sync/diff` — it snapshots local + Shopify catalogs once and feeds all three builders, so the page render costs one Shopify walk instead of three. Each apply runs inside `withSyncLock` — a MySQL `GET_LOCK('shopify_sync', 5)` bound to the wrapping DB transaction so concurrent operators serialise on the same lock. The lock is released explicitly before the connection returns to the pool. **Never call link/push/pull writes outside `withSyncLock`** — atomicity guarantees depend on the lock+trx pair.
...
- Images flow one way: pulled from Shopify into `variants.image_url` / `products.image_url` on first link (denormalised "canonical" image). The UI renders them via the `ImageZoom` component, which calls `shopifyThumb(url, width)` to ask the Shopify CDN for a sized thumbnail — never embed raw Shopify CDN URLs directly into `<img>` tags.
