  ...
  <div class="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto">
    <PageHeader :title="product.name" :description="product.category?.name ?? undefined">
      <template #actions>
        ...
        <Link route="products.edit" :params="{ id: product.id }" class="btn-secondary">{{
    ...
    <div class="card overflow-hidden mb-8">
      <header class="px-5 py-3 border-b border-slate-100 flex items-center justify-between">
        ...
        <Link route="stock.adjust.create" class="btn-secondary text-xs">{{
          $t('nav.items.adjustStock')
        }}</Link>
      </header>
